...
public static class ConfigHandler
{
    ...
    public static Config? LoadConfig()
    {
        ...
        config.CoreTypeItem ??= [];
        if (config.CoreDefaultsVersion < 1)
        {
            var hysteria2Core = config.CoreTypeItem.FirstOrDefault(item => item.ConfigType == EConfigType.Hysteria2);
            if (hysteria2Core is null)
            {
                config.CoreTypeItem.Add(new CoreTypeItem { ConfigType = EConfigType.Hysteria2, CoreType = ECoreType.sing_box });
            }
            else if (hysteria2Core.CoreType == ECoreType.Xray)
            {
                hysteria2Core.CoreType = ECoreType.sing_box;
            }
            config.CoreDefaultsVersion = 1;
        }
        ...
        config.TunModeItem.RouteExcludeAddress = CoreConfigContextBuilder.NormalizeTunRouteExcludeAddresses(
            config.TunModeItem.RouteExcludeAddress,
            config.SystemProxyItem.SystemProxyExceptions);
