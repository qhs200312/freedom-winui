...
public class CoreConfigContextBuilder
{
    ...
    public static async Task<CoreConfigContextBuilderResult> Build(Config config, ProfileItem node)
    {
        ...
        if (context.IsTunEnabled)
        {
            ...
            {
                var network = IPNetwork2.Parse(addr);
                if (routeExcludeAddressSet.Add(network.ToString()))
                {
                    routeExcludeAddressList.Add(addr);
                }
            }

            foreach (var addr in GetInvalidTunRouteExcludeAddresses(context.AppConfig.TunModeItem.RouteExcludeAddress))
            {
                validatorResult.Warnings.Add(string.Format(ResUI.MsgTunRouteExcludeInvalidAddress, addr));
            }

            foreach (var addr in GetSystemProxyRouteExcludeAddresses(context.AppConfig.SystemProxyItem.SystemProxyExceptions))
            {
                var network = IPNetwork2.Parse(addr);
                if (routeExcludeAddressSet.Add(network.ToString()))
                {
    ...
    internal static List<string> GetSystemProxyRouteExcludeAddresses(string? exceptions)
    {
        return GetRouteExcludeAddresses(exceptions.IsNullOrEmpty() ? [] : [exceptions], out _);
    }

    internal static List<string> GetTunRouteExcludeAddresses(IEnumerable<string>? addresses)
    {
        return GetRouteExcludeAddresses(addresses, out _);
    }

    internal static List<string> NormalizeTunRouteExcludeAddresses(IEnumerable<string>? addresses, string? systemProxyExceptions)
    {
        var result = GetTunRouteExcludeAddresses(addresses);
        if (result.Count == 0)
        {
            result = GetSystemProxyRouteExcludeAddresses(systemProxyExceptions);
        }

        return result.Distinct(StringComparer.OrdinalIgnoreCase).ToList();
    }

    private static List<string> GetInvalidTunRouteExcludeAddresses(IEnumerable<string>? addresses)
    {
        _ = GetRouteExcludeAddresses(addresses, out var invalidAddresses);
        return invalidAddresses;
    }

    private static List<string> GetRouteExcludeAddresses(IEnumerable<string>? values, out List<string> invalidAddresses)
    {
        ...
        var entries = values.SelectMany(value =>
            value.Split([';', ',', '\r', '\n'], StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
        foreach (var entry in entries)
        {
            try
            {
                IPNetwork2.Parse(entry);
                result.Add(entry);
                continue;
            }
            catch
            {
                // System proxy exceptions also support domains; TUN route exclusions do not.
            }

            var wildcardAddress = ConvertIPv4WildcardToCidr(entry);
            if (wildcardAddress != null)
            {
                result.Add(wildcardAddress);
                continue;
            }
    ...
    private static string? ConvertIPv4WildcardToCidr(string value)
    {
        ...
        var wildcardIndex = Array.IndexOf(parts, "*");
        if (wildcardIndex is <= 0 or > 3 || parts.Skip(wildcardIndex).Any(x => x != "*"))
        {
            return null;
        }
        ...
        return $"{string.Join('.', octets)}/{wildcardIndex * 8}";
    }
