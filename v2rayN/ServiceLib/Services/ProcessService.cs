...
public class ProcessService : IDisposable
{
    ...
    private readonly bool _optimizeForThroughput;
    ...
    public ProcessService(
        ...
        bool optimizeForThroughput = false)
    {
        _updateFunc = updateFunc;
        _optimizeForThroughput = optimizeForThroughput;
    ...
    public async Task StartAsync(string pwd = null)
    {
        ...
        if (_optimizeForThroughput && Utils.IsWindows())
        {
            try
            {
                var processorCount = Environment.ProcessorCount;
                var affinityBits = IntPtr.Size * 8;
                if (processorCount > 0 && processorCount <= affinityBits)
                {
                    var affinity = processorCount == 64 ? -1L : (long)((1UL << processorCount) - 1);
                    _process.ProcessorAffinity = (nint)affinity;
                }
                _process.PriorityClass = ProcessPriorityClass.AboveNormal;
            }
            catch (Exception ex)
            {
                Logging.SaveLog("Unable to apply core process performance settings", ex);
            }
        }
    ...
    public async Task StopAsync()
    {
        ...
        try
        {
            ...
            {
                Logging.SaveLog($"Timed out waiting for process {_process.Id} to exit.");
            }
        }
        catch (Exception ex)
        {
